    ...
    if (empty($name) || empty($description) || empty($price) || empty($stock_quantity)) {
        ...
        try {
            ...
            if (empty($error_message)) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE products 
                    SET name = ?, description = ?, price = ?, category = ?, Sub_Cat = ?, 
                        stock_quantity = ?, image_path = ?, size_data = ?
...
<body>
    ...
    <div class="container edit-product">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group" id="sizeGroup" style="display: <?php echo $product['category'] === 'clothes' ? 'block' : 'none'; ?>;">
                <label>Sizes and Quantities*</label>
                <div id="sizeContainer">
                    <?php
                    $size_data = json_decode($product['size_data'], true);
                    if ($size_data) {
                        foreach ($size_data as $size => $quantity) {
                            echo '<div class="size-row">';
                            echo '<select class="size-select" name="sizes[]">';
                            echo '<option value="XS" ' . ($size === 'XS' ? 'selected' : '') . '>XS</option>';
                            echo '<option value="S" ' . ($size === 'S' ? 'selected' : '') . '>S</option>';
                            echo '<option value="M" ' . ($size === 'M' ? 'selected' : '') . '>M</option>';
                            echo '<option value="L" ' . ($size === 'L' ? 'selected' : '') . '>L</option>';
                            echo '<option value="XL" ' . ($size === 'XL' ? 'selected' : '') . '>XL</option>';
                            echo '</select>';
                            echo '<input type="number" class="size-quantity" name="size_quantities[]" min="0" value="' . $quantity . '" required>';
                            echo '<button type="button" class="btn-remove-size" onclick="removeSize(this)">×</button>';
                            echo '</div>';
                        }
                    }
                    ?>
                </div>
                <button type="button" class="btn-add-size" onclick="addSize()">Add Size</button>
            </div>
            
            <div class="form-group" id="stockGroup" style="display: <?php echo $product['category'] === 'clothes' ? 'none' : 'block'; ?>;">
    ...
    <script>
        ...
        function addSize() {
            ...
            removeButton.className = 'btn-remove-size';
            removeButton.textContent = '×';
            removeButton.onclick = function() { removeSize(this); };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            updateSubcategories();
            const subcategorySelect = document.getElementById('subcategory');
            const currentSubcategory = '<?php echo htmlspecialchars($product['Sub_Cat']); ?>';
            
            // Check if current subcategory is in the list
            let found = false;
            ...
            // If not found, it's a custom subcategory
            if (!found) {
                subcategorySelect.value = 'custom';
                handleSubcategoryChange();
                document.getElementById('customSubcategory').value = currentSubcategory;
            }
        });
    </script>
</body>
